...
public:
	...
	void operator=(const BinarySearchTree<T>& obj)
	{
		MakeEmpty();
		AddInOrderTraversal(obj.GetRoot());
	}
	/**
	...
	*/
	...
	/**
	*	@brief	Tree에서 바꾸고자 하는 node를 찾고 지움
	*/
	void Replace(T item);
	...
	/**
	*	@brief	입력한 값의 node를 Tree에서 Get
	*/
	int Get(T& item);


	void AddInOrderTraversal(Node<T>* root)
	{
		if (root != NULL)								// root가 존재하는 경우
		{
			AddInOrderTraversal(root->left);			// root의 왼쪽으로 가서 다시 InOrder 함수 호출
			Add(root->data);							// root 출력
			AddInOrderTraversal(root->right);			// root의 오른쪽으로 가서 다시 InOrder 함수 호출
		}
	}
...
template <typename T>
int BinarySearchTree<T> ::Get(T& item)
{
	bool found;
...
template <typename T>
void BinarySearchTree<T>::Replace(T item)
{
	Delete(root, item);
	Insert(root, item);
